 ...
 * i18n.js —— 中英文切换
 * ------------------------------------------------------------
 * 依赖：core.js / i18n-dict.js
 * 对外：App.i18n.lang() / set('en') / toggle() / t(key) / refresh()
 * 事件：切换后派发 langchange，detail = { lang: 'zh' | 'en' }
 *
 * 本版修改：
 *   ★ apply() 里给英文内容打 lang="en"
 *     屏幕阅读器在中文页面里读英文专有名词（如基金名、HUANG）
 *     时，有 lang 标记会切换发音引擎，避免读成乱码。
 * ============================================================ */
(function (App) {
  'use strict';
  ...
  function apply(lang) {
    ...
    App.$$('[data-i18n-aria]').forEach(function (el) {
      ...
      if (d[key] != null) {
        ...
        /* aria-label 是读给 AT 听的，语言标记同样有意义 */
        if (isEn) el.setAttribute('lang', 'en');
        else      el.removeAttribute('lang');
      }
    });
