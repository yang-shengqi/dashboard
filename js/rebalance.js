 ...
 *
 * 2026-09-19：
 *   ★ 核心算法抽成纯函数 allocateForHolder()（便于自测）
 *   ★ 新增 selfTest() + requires
 * ============================================================ */
(function (App) {
  'use strict';
   ...
   * 参数：
   *   cash    可用现金（HKD）
   *   needs   { total, rows: { code: { target, amountHKD } } }
   *   minMap  { code: 最低起投（原币） }
   *   rates   { code: 汇率（1 原币 = ? HKD） }
   *
   * 返回：
   *   { code: { cls: 'buy'|'dash', amount } }
   *   cls='buy' 时 amount>0；cls='dash' 时 amount=0
   *
   * 顺序：按 CCY_ORDER，先满足的币种先吃现金
   * ============================================================ */
  ...
  function selfTest() {
    ...
    /* --- 测试 7：minHKD 换算（USD 最低 1000，汇率 7.85） --- */
    ...
    /* need = 5000，minHKD = 7850 > 5000（缺口本身小于最低起投）→ dash */
    if (r7.USD.cls !== 'dash')
  ...
  function adviceText(adv) {
    if (!adv || adv.cls === 'dash' || adv.amount <= 0) {
      return { cls: 'dash', text: '—' };
    }
    return { cls: 'buy', text: fmt(adv.amount) + ' HKD' };
  }
