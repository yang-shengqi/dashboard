 ...
 *
 * 本版修改：
 *   ★ 年份折叠头从 <div> 改为 <button>
 *     —— 原 <div class="ds-year-header"> 无 tabindex，
 *       键盘无法展开/折叠；现在用 button 天然可达
 *   ★ .ds-year-arrow 改用 CSS transform 旋转（去掉 ▶ 文字）
 *   ★ getHolderTotalValue / getHolderCurrencyValue 补
 *     value=0 → cost 兜底（与 split-monitor / asset-overview /
 *     deviation 口径统一）
 * ============================================================ */
(function (App) {
  'use strict';
        ...
        pageHolderYears.forEach(function (year) {
          ...
          TEN_CURRENCIES.forEach(function (ccy) {
            ...
            html += '<div class="ds-item" role="row">' +
              ...
              '<span class="ds-ccy" role="cell">' + (CURRENCY_LABELS[ccy] || ccy) + '</span>' +
