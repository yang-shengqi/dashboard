 ...
 *   ★ 点击「刷新汇率」按钮时弹提示（成功 / 失败 / 获取中）
 *
 * 2026-09-19：
 *   ★ 抽出纯函数 pickRate(row, autoRates) —— 三级优先逻辑
 *   ★ 新增 selfTest() + requires
 *   ★ DEFAULT_CURRENCIES.preset 从 App.constants 读
 * ============================================================ */
(function (App) {
  'use strict';
  ...
  /* ★ 2026-09-19：preset 从 App.constants 读，与 core / hk-accounts 同源 */
  ...
  function syncRates() {
    collection.all().forEach(function (r) {
      ...
      var effective = pickRate(r, autoRates);   /* ★ 复用纯函数 */
  ...
  function currentRate(r) {
    return pickRate(r, autoRates);   /* ★ 复用纯函数 */
  }
