 ...
 *
 * 本版修改：
 *   ★ 汇率输入框补 aria-label（原仅 placeholder/无标签）
 *   ★ 汇率输入框补 form 相关属性（inputmode 数字键盘）
 *   ★ 其余保持
 * ============================================================ */
(function (App) {
  'use strict';
  ...
  function renderHolders(overall) {
    ...
    grid.innerHTML = list.map(function (h) {
      ...
      var bp = d.bochkParts || {};
      var bochkRows = [];
      var bochkHkd = bp.hkdCurrent + bp.hkdFixed;
      var bochkUsd = bp.usdCurrent + bp.usdFixed;
  ...
  App.hkAccounts = {
    init: function () {
      ...
      var input = App.$('#exchangeRateInput');
      if (input) {
        /* ★ 补 aria-label —— 原只有 placeholder/无标签 */
        if (!input.getAttribute('aria-label')) {
          input.setAttribute('aria-label', T('hkAcc.rate', '汇率') + ' CNY → HKD');
        }
        /* ★ 手机端弹数字键盘 */
