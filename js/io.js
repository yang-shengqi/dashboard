 ...
 * io.js —— 导入导出
 * ------------------------------------------------------------
 * 依赖：core.js（App.storage / App.utils）
 *       xlsx.full.min.js（第三方，仅 XLSX 功能需要）
 * 对外：
 *   App.io.exportCSV(rows, filename, columns)
 *   App.io.exportJSON(data, filename)
 *   App.io.exportXLSX(rows, filename, sheetName, columns)
 *   App.io.importCSV(file, options)   → Promise<Array>
 *   App.io.importJSON(file)           → Promise<Object|Array>
 *   App.io.importXLSX(file, options)  → Promise<Array>
 *   App.io.exportCollection(col, filename, format)
 *   App.io.importToCollection(file, col, format)
 * ------------------------------------------------------------
 * columns 格式（可选）：
 *   [{ key: 'id', label: '编号' }, { key: 'value', label: '市值' }]
 *   不传则自动取第一行的所有 key 作为表头
 * ============================================================ */
(function (App) {
  'use strict';
