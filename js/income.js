  ...
  function renderAccountGroup(acct, rows, year, holder) {
    ...
    ccyList.forEach(function (ccy) {
      ...
      if (isSchwab) {
        ...
        /* aria-label 用代码，读屏更精确 */
        function cellAria(field) {
          return acctLabel + ' ' + year + ' ' + ccy + ' ' +
                 T('income.col.' + field, field);
        }
        ...
        html += '<div class="ds-item income-schwab-row" data-key="' + k + '">' +
          '<span>' + label + '</span>' +
          '<span><input class="income-cell" type="number" step="0.01" inputmode="decimal" ' +
              'data-field="dividend" value="' + vDiv + '" placeholder="—" ' +
              'aria-label="' + App.utils.escapeHTML(cellAria('dividend')) + '" /></span>' +
          '<span><input class="income-cell" type="number" step="0.01" inputmode="decimal" ' +
              'data-field="interest" value="' + vInt + '" placeholder="—" ' +
              'aria-label="' + App.utils.escapeHTML(cellAria('interest')) + '" /></span>' +
          '<span><input class="income-cell" type="number" step="0.01" inputmode="decimal" ' +
              'data-field="diff" value="' + vDif + '" placeholder="—" ' +
              'aria-label="' + App.utils.escapeHTML(cellAria('diff')) + '" /></span>' +
        '</div>';
      } else {
