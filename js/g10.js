 ...
 * ★ 阈值（THRESHOLD）与进度条刻度线联动，改一处即全变
 * ★ 国旗走本地图片（images/flags/xxx.png）
 * ★ 卡片顶部：国旗在左，偏离值在右
 * ★ 阈值说明移到面板头（HTML 里，不在这里渲染）
 * ============================================================ */
(function (App) {
  'use strict';

  var FLAG_BASE = 'images/flags/';   /* ★ 本地国旗图目录 */
  ...
  var MAX_ABS   = 30;   /* 进度条满格对应的偏离度（±%） */
  var THRESHOLD = 5;    /* 高估/低估判定阈值（±%） */
  ...
  /* 阈值线在进度条里的位置（百分比，中心=50%） */
  function markPositions() {
    var offset = (THRESHOLD / MAX_ABS) * 50;   /* 5/30*50 ≈ 8.33% */
  ...
  function render() {
    ...
    grid.innerHTML = data.map(function (r) {
      ...
      var flagHtml = '<img class="g10-flag-img" ' +
        ...
        'alt="' + App.utils.escapeHTML(r.code) + '" ' +
        'loading="lazy" ' +
        'onerror="this.style.display=\'none\'">';

      return '<div class="g10-card">' +
        '<div class="g10-top">' +
          flagHtml +
          '<span class="g10-dev ' + cls + '">' + sign + dev.toFixed(1) + '%</span>' +
        '</div>' +
  ...
  App.g10 = {
    init: function () {
      ...
      if (saved && Array.isArray(saved) && saved.length) {
        data = saved;
        console.log('[g10] 从缓存恢复：', data.length, '条');
      }
      render();
