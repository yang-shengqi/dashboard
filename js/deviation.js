...
(function (App) {
  ...
  function render() {
    ...
    html += r.rows.map(function (row) {
      ...
      /* ★ 货币标签跟随语言 */
      var label = ccyLabel(row.code);

      return '<div class="dev-row ' + cls + '" role="row">' +
        '<span class="dev-code" role="cell">' + label + '</span>' +
    ...
    if (h.value > 0 || h.valueHKD > 0) {
      /* ★ HKD 行标签也跟随语言 */
      var hkdLabel = ccyLabel('HKD');

      html += '<div class="dev-row hkd-note" role="row">' +
        '<span class="dev-code" role="cell">' + hkdLabel + '</span>' +
